...
namespace BreakoutParty.Font
{
    ...
    /// </summary>
    static class BitmapFontLoader
    {
        /// <summary>
        /// Font cache.
        /// </summary>
        private static Dictionary<string, BitmapFont> _FontCache =
        ...
        /// </returns>
        public static BitmapFont LoadBitmapFont(this ContentManager content, string assetName)
        {
            BitmapFont result;

            // Try to get the font from cache
            if (_FontCache.TryGetValue(assetName, out result))
                return result;

            // Actual font file ends with "fnt"
            string fontFile = Path.Combine(content.RootDirectory, assetName + ".fnt");

            // Read first line to find out what kind of file it is
            string line;
            using (FileStream file = File.OpenRead(fontFile))
            ...
            {
                result = LoadFontFromTextFile(fontFile);
            }
            else if (line.StartsWith("<?xml"))
            {
                result = LoadFontFromXmlFile(fontFile);
            }
            else
            {
            ...
            // Load texture
            result.FontTexture = content.Load<Texture2D>(assetName);

            // Add to cache
            _FontCache.Add(assetName, result);
        ...
        /// <param name="font">The <see cref="BitmapFont"/></param>
        /// <param name="text">The text to draw.</param>
        /// <param name="position">The position to draw to.</param>
        /// <param name="color">The color tint.</param>
        public static void DrawString(this SpriteBatch batch,
            BitmapFont font,
            string text,
            Vector2 position,
            Color color)
        {
            Point size = font.MeasureFont(text);
                    ...
                    switch (character)
                    {
                        ...
                        default:
                            ...
                            DrawCharacter(batch,
                                font.FontTexture,
                                data,
                                new Vector2(x + data.Offset.X + kerning, y + data.Offset.Y),
                                color);
        ...
        private static void DrawCharacter(SpriteBatch batch,
            Texture2D texture,
            Character character,
            Vector2 position,
            Color color)
        {
            batch.Draw(texture, position, character.Bounds, color);
        }
        ...
        /// A <see cref="BitmapFont"/> containing the loaded data.
        /// </returns>
        private static BitmapFont LoadFontFromTextFile(string fileName)
        {
            BitmapFont font;
        ...
        /// A <see cref="BitmapFont"/> containing the loaded data.
        /// </returns>
        private static BitmapFont LoadFontFromXmlFile(string fileName)
        {
            BitmapFont font;
